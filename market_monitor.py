...
class MarketMonitor:
    ...
    def analyze_options_chain(self, symbol: str, price: float) -> Dict:
        """
        Fetch real options chain data from yfinance and extract Greeks,
        put/call ratios, and unusual activity for ATM options.
        """
        result = {
            "has_options": False,
            "atm_call": {},
        ...
        try:
            ...
            # Pick the nearest expiry 25-50 days out
            today = date.today()
            target_exp = None
            ...
            # Fallback: nearest expiry at least 7 days out
            if not target_exp:
                for exp_str in expirations:
                    exp_date = datetime.strptime(exp_str, "%Y-%m-%d").date()
                    dte = (exp_date - today).days
    ...
    def analyze_symbol(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Estimated Greeks (always available)
            greeks = {
                "est_delta_call": 0.50,
                "est_delta_put": -0.50,
            ...
            # Real options chain data (ATM calls/puts with live Greeks)
            ...
            if high_iv_opts:
                ...
                for s in sorted(high_iv_opts, key=lambda x: x["iv_rank"], reverse=True)[:10]:
                    ...
                    lines.append(
                        ...
                        f"ATM Call ${atm_c.get('bid', 0):.2f}/{atm_c.get('ask', 0):.2f} "
                        f"ATM Put ${atm_p.get('bid', 0):.2f}/{atm_p.get('ask', 0):.2f} "
                        f"Trend:{s['trend']}"
                    )
                lines.append("")
    ...
    def generate_html_report(self, results: List[Dict], changes: List[Dict]) -> str:
        ...
        for s in results[:25]:
            ...
            call_iv = s["greeks"].get("atm_call_iv", "-")
...
<body>
    ...
    <table>
        <tr>
            ...
            <th>Stoch K/D</th><th>HV20</th><th>ATM IV</th><th>Theta</th>
            <th>P/C Ratio</th><th>Trend</th><th>Signals</th>
        </tr>
        {top_rows}
    </table>
    ...
    def _html_options_section(self, results: List[Dict]) -> str:
        ...
        for s in high_iv:
            ...
            rows += f"""<tr>
                ...
                <td>{atm_c.get('volume', 0):,}</td>
                <td>${atm_p.get('bid', 0):.2f}/{atm_p.get('ask', 0):.2f}</td>
                <td>{atm_p.get('volume', 0):,}</td>
        ...
        return f"""
    <h2>Options Opportunities — Premium Selling Candidates</h2>
    <table>
        <tr>
            <th>Symbol</th><th>Price</th><th>IV Rank</th><th>HV20</th>
            <th>ATM Call Bid/Ask</th><th>Call Vol</th>
            <th>ATM Put Bid/Ask</th><th>Put Vol</th>
            <th>P/C Ratio</th><th>Total OI</th><th>Flag</th>
        </tr>
        {rows}
    </table>"""
